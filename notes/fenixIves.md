...
<!-- Content_START -->
# 2026-03-25
<!-- DAILY_CHECKIN_2026-03-25_START -->
# 📘 区块链项目测试部署完整流程指南
...
<!-- DAILY_CHECKIN_2026-03-25_END -->

# 2026-03-24
<!-- DAILY_CHECKIN_2026-03-24_START -->

## 统一监听方案

### **同一链上的多种事件监听**

可以在一个Reactive合约中同时监听：

1\. **DEX巨鲸交易**

-   **Uniswap V3**: `Swap` 事件 (topic\_0: `0xc42079f94a6350d7e6235f29174924f928cc2ac818eb64fed8004e115fbcca67`)
    
-   **Uniswap V2**: `Swap` 事件 (topic\_0: `0xd78ad95fa46c994b6551d0da85fc275fe613ce37657fb8d5e3d130840159d822`)
    

2\. **大额转账**

-   **ERC20 Transfer**: `Transfer` 事件 (topic\_0: `0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef`)
    
-   **ETH Transfer**: 监听大额ETH转账到特定地址
    

3\. **DeFi协议操作**

-   **Aave**: `Borrow`, `Deposit`, `LiquidationCall` 事件
    
-   **Compound**: `Borrow`, `Mint`, `LiquidationBorrow` 事件
    

### **实现架构**

```
contract WhaleMonitorReactive {
    // 同时订阅多个事件
    function getPausableSubscriptions() external pure returns (Subscription[] memory) {
        Subscription[] memory subs = new Subscription[](5);
        
        // Uniswap V3 Swap
        subs[0] = Subscription(ETHEREUM_CHAIN_ID, UNISWAP_V3_POOL, SWAP_TOPIC);
        
        // USDC Transfer
        subs[1] = Subscription(ETHEREUM_CHAIN_ID, USDC_ADDRESS, TRANSFER_TOPIC);
        
        // USDT Transfer  
        subs[2] = Subscription(ETHEREUM_CHAIN_ID, USDT_ADDRESS, TRANSFER_TOPIC);
        
        // Aave Borrow
        subs[3] = Subscription(ETHEREUM_CHAIN_ID, AAVE_LENDING_POOL, BORROW_TOPIC);
        
        // Compound Liquidation
        subs[4] = Subscription(ETHEREUM_CHAIN_ID, COMPOUND_COMPTROLLER, LIQUIDATION_TOPIC);
...
-   每条链需要单独的Reactive合约部署
    

**同链不同协议** (可以统一)

-   **Ethereum上**: Uniswap + Aave + Compound + USDT/USDC转账
...
这样你可以用最少的成本实现最全面的巨鲸监听系统。
<!-- DAILY_CHECKIN_2026-03-24_END -->
...
CRON 是 Reactive Network 提供的**链上定时器机制**，允许智能合约按固定区块间隔自动执行任务。这解决了传统区块链无法主动触发执行的问题。

## 2\. 核心架构组件

### 2.1 系统合约 (SystemContract)

-   **位置**: Reactive Network 核心合约
...
-   根据区块号发出不同频率的 CRON 事件
    
-   监听并执行 Reactive 合约的 Callback 请求
...
```
// 不同频率的 CRON 主题（事件标识符）
Cron1    → 0xf02d6ea5c22a71cffe930a4523fcb4f129be6c804db50e4202fb4e0b07ccb514  // 每1个区块
Cron10   → 0x04463f7c1651e6b9774d7f85c85bb94654e3c46ca79b0c16fb16d4183307b687  // 每10个区块  
Cron100  → 0xb49937fb8970e19fd46d48f7e3fb00d659deac0347f79cd7cb542f0fc1503c70  // 每100个区块
Cron1000 → 0xe20b31294d84c3661ddc8f423abb9c70310d0cf172aa2714ead78029b325e3f4  // 每1000个区块
Cron10000→ 0xd214e1d84db704ed42d37f538ea9bf71e44ba28bc1cc088b2f5deca654677a56  // 每10000个区块
```

### 2.3 Reactive 合约 (BasicCronContract)

-   **继承**: `AbstractPausableReactive`
...
![](https://cdn.nlark.com/yuque/__mermaid_v3/3d83dd3caf17967d464f655720e66ad5.svg)

**代码位置**: `CronDemo.sol:28-48`

```
constructor(address _service, uint256 _cronTopic) payable {
    ...
    if (!vm) {
        service.subscribe(
            ...
            address(service),  // 系统合约地址
            _cronTopic,       // CRON 主题
            REACTIVE_IGNORE,   // 忽略 topic1
            REACTIVE_IGNORE,   // 忽略 topic2
            REACTIVE_IGNORE    // 忽略 topic3
        );
    }
}
```
...
sequenceDiagram
    ...
    participant BC as BasicCronContract
    
    V->>SC: cron() [每个区块]
    SC->>SC: 检查区块号 % 间隔 == 0?
    alt 区块号可被整除
        SC->>RN: 发出 Cron 事件
        RN->>BC: react(log) [虚拟机调用]
    end
```

**触发条件**:

-   验证者调用 `SystemContract.cron()`
    
-   当前区块号能被所选间隔整除
    
-   例如：Cron10 在区块号 10, 20, 30... 时触发
    

### 3.3 Reactive 响应阶段

**代码位置**: `CronDemo.sol:71-84`
...
function react(LogRecord calldata log) external vmOnly {
    ...
    if (log.topic_0 == CRON_TOPIC) {
        ...
        // 3. 发出回调请求
        emit Callback(
            block.chainid,                    // 目标链 ID
            address(this),                    // 回调目标合约
            GAS_LIMIT,                        // Gas 限制
            abi.encodeWithSignature("callback()") // 调用数据
        );
    }
}
```
...
-   `vmOnly` 修饰符确保只有 Reactive 虚拟机能调用
    
-   `topic_0 == CRON_TOPIC` 确保只响应特定频率的事件
    
-   `Callback` 事件是**异步请求**，不是立即执行
    

### 3.4 Callback 执行阶段

![](https://cdn.nlark.com/yuque/__mermaid_v3/5869e95e55aa74896ce6949fa8dc70b5.svg)

**Payload 解析**:

```
// payload = abi.encodeWithSignature("callback()")
// 等价于:
bytes4 selector = bytes4(keccak256("callback()")); // 0x12345678
// 无参数
```
...
```
// 1. 订阅阶段
service.subscribe(chain_id, contract_address, topic, ...)

// 2. 响应阶段  
function react(LogRecord log) external vmOnly {
    if (log.topic_0 == subscribed_topic) {
        // 处理逻辑
    }
}
```
...
```
// 1. 发出回调请求
emit Callback(chain_id, target_contract, gas_limit, payload);

// 2. 系统异步执行
// Reactive Network 监听 Callback 事件并执行实际调用
```

### 4.3 权限控制模式

-   `vmOnly`: 只有 Reactive 虚拟机能调用 `react()`
    
-   `authorizedSenderOnly`: 只有授权地址能调用 `callback()`
...
```
┌─────────────────┐    ┌─────────────────┐
│ BasicCronContract │    │  SystemContract │
│─────────────────│    │─────────────────│
│ • react()       │◄──►│ • cron()        │
│ • callback()    │    │ • 执行回调       │
└─────────────────┘    └─────────────────┘
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│ ReactiveContract │    │  SystemContract │    │ CallbackContract │
│─────────────────│    │─────────────────│    │─────────────────│
│ • react()       │◄──►│ • 监听事件       │◄──►│ • callback()     │
│ • 发出Callback   │    │ • 执行回调       │    │ • 业务逻辑       │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```
...
```
contract MyCronContract is AbstractPausableReactive {
    // 1. 状态变量
    ...
    // 4. 回调函数
    function callback() external authorizedSenderOnly {
        // 实际业务逻辑
    }
    
    // 5. 管理函数
    function pause() external onlyOwner { isPaused = true; }
    function resume() external onlyOwner { isPaused = false; }
}
```
...
function testCronTrigger() public {
    ...
    // 验证结果
    assertEq(contract.lastCronBlock(), block.number);
}
```

### 9.2 网络调试

-   **事件监听**: 监听 Cron 和 Callback 事件
...
Reactive Network 的 CRON 机制提供了一个强大的链上定时器解决方案，通过以下关键特性实现了智能合约的自动化执行：

1.  **确定性触发**: 基于区块号的可靠触发机制
...
CRON 例子虽然简单，但展示了 Reactive Network 的核心设计理念：**事件驱动 + 异步回调**，这是理解整个 Reactive Network 架构的关键入口。
...
# **Uniswap V2** 止损订单Demo完整部署指南（含常见错误与解决方案）

**一、.env 环境变量配置**

以下是正确的 **.env** 配置模板，每个变量的说明和常见错误都已标注。
...
```
DESTINATION_PRIVATE_KEY=0x你的私钥
```
...
```
REACTIVE_PRIVATE_KEY=0x你的私钥
```

```
# ── 回调代理地址（Ethereum Sepolia 专用）───────────────────
```
...
```
TOKEN0_ADDR=      # 步骤1：Pair认定的token0地址（地址较小的）
```

```
TOKEN1_ADDR=      # 步骤1：Pair认定的token1地址（地址较大的）
```

```
UNISWAP_V2_PAIR_ADDR=  # 步骤2：从PairCreated事件获取
```

```
CALLBACK_ADDR=    # 步骤3：回调合约的Deployed to地址
```

```
CLIENT_WALLET=0x你的钱包地址
```
...
```
DIRECTION_BOOLEAN=true   # true=卖token0买token1，false=相反
```
...
```
EXCHANGE_RATE_NUMERATOR=900  # 阈值=900/1000=0.9（需低于当前汇率才触发）
```

**1.2 关键变量易错点**

| 变量名 | 常见错误 |
| DESTINATION_RPC | ❌ 填成 Base Sepolia (https://sepolia.base.org)✅ 必须是 Ethereum Sepolia |
| DESTINATION_CALLBACK_PROXY_ADDR | ❌ 填成 0x0000...fffFfF（Reactive自己的地址）✅ Ethereum Sepolia 专用地址 |
| TOKEN0_ADDR / TOKEN1_ADDR | ❌ 按自己部署顺序填写✅ 以 Pair 合约 token0()/token1() 返回值为准 |
| UNISWAP_V2_PAIR_ADDR | ❌ 填成回调合约的 Deployed to 地址✅ 从 PairCreated 事件中获取 |
| CALLBACK_ADDR | ❌ 填成 Uniswap Factory 地址✅ 步骤3回调合约的 Deployed to 地址 |
| EXCHANGE_RATE_NUMERATOR | ❌ 随意填写导致阈值高于当前汇率，永不触发✅ 阈值需低于当前池子汇率 |

**二、完整部署流程**

**步骤 1 — 部署测试代币**

部署两个 **ERC-20** 测试代币，每个向部署者铸造 **100** 个单位。
...
  --constructor-args TK2 TK2
```

⚠️  两次部署的 **Deployed to** 地址暂时保存，但不要直接设为 **TOKEN0/TOKEN1**，需等步骤**2**创建**Pair**后再确认顺序。

**步骤 2 — 创建 Uniswap V2 Pair**

用 **Uniswap V2 Factory** 创建交易对。**Factory** 地址固定为：
...
⚠️  执行后去 **Sepolia Etherscan** 的交易详情 **→ Event Logs**，找 **PairCreated** 事件，获取 **Pair** 地址。

然后查询 **Pair** 实际认定的 **token** 顺序（**Uniswap** 按地址大小自动排序）：
...
```
export TOKEN0_ADDR=<token0()返回的地址>
```

```
export TOKEN1_ADDR=<token1()返回的地址>
```

**步骤 3 — 部署回调合约**

⚠️  必须先确认 **DESTINATION\_CALLBACK\_PROXY\_ADDR** 已设置为正确地址，否则回调永远无法验证。
...
将输出的 **Deployed to** 地址设为 **CALLBACK\_ADDR**：

```
export CALLBACK_ADDR=<Deployed to 地址>
```

**步骤 4 — 向池中添加流动性**

向 **Pair** 合约转入两种代币，然后铸造 **LP** 代币。

```
cast send $TOKEN0_ADDR 'transfer(address,uint256)' \
...
cast send $TOKEN1_ADDR 'transfer(address,uint256)' \
...
设置好阈值参数后部署到反应式网络（**Lasna Testnet**）。

⚠️  **EXCHANGE\_RATE\_NUMERATOR/DENOMINATOR** 的阈值必须低于当前池子汇率才会触发！用 **getReserves()** 确认当前汇率后再设置。

查询当前汇率（**reserve1/reserve0** 即为汇率）：
...
```
forge create --broadcast --rpc-url $REACTIVE_RPC \

  --private-key $REACTIVE_PRIVATE_KEY \

  src/demos/uniswap-v2-stop-order/UniswapDemoStopOrderReactive.sol:UniswapDemoStopOrderReactive \
...
授权回调合约花费要出售的代币。**DIRECTION\_BOOLEAN=true** 时出售 **token0**，否则出售 **token1**。

```
cast send $TOKEN0_ADDR 'approve(address,uint256)' \

  --rpc-url $DESTINATION_RPC --private-key $DESTINATION_PRIVATE_KEY \

  $CALLBACK_ADDR 1000000000000000000
```

**步骤 7 — 触发汇率变化**

先转代币进 **Pair**，再执行 **swap** 造成汇率变动，触发反应式合约监听的 **Sync** 事件。
...
  $UNISWAP_V2_PAIR_ADDR 20000000000000000



cast send $UNISWAP_V2_PAIR_ADDR 'swap(uint,uint,address,bytes calldata)' \

  --rpc-url $DESTINATION_RPC --private-key $DESTINATION_PRIVATE_KEY \

  0 5000000000000000 $CLIENT_WALLET "0x"
```

✅  等待 **1-2** 分钟后，去 **Etherscan** 查 **CALLBACK\_ADDR** 的 **Events**，出现 **Stop** 事件表示成功！

验证链接：

[**https://sepolia.etherscan.io/address/<CALLBACK\_ADDR>#events**](https://sepolia.etherscan.io/address/<CALLBACK_ADDR>#events)

**三、常见错误与解决方案**

**3.1 编译错误**

**Error 3881：NatSpec 参数名不匹配**

原因：注释里的 **@param sender** 对应的函数参数被注释掉了（**address /\* sender \*/**），编译器找不到该参数名。

解决：删除注释中对应的 **@param sender** 一行，或恢复函数参数名。
...
| 场景 | 原因 | 解决方案 |
| mint() revert | 代币未转入 Pair，或地址错误 | 先 transfer 两种代币到 Pair 再 mint |
| getReserves() revert | 地址不是 Uniswap V2 Pair 合约 | 用 PairCreated 事件获取正确地址 |
| balanceOf revert | 代币合约在该网络上不存在 | 确认 RPC 网络与部署网络一致 |

**3.3 网络错误**

**contract does not have any code**

原因：**RPC** 指向的网络与合约实际部署的网络不一致。

解决：确认 **DESTINATION\_RPC** 必须是 **Ethereum Sepolia**，不能是 **Base Sepolia** 或其他网络。

**3.4 回调不触发**

| 原因 | 排查方法 | 解决方案 |
| CALLBACK_PROXY 地址错误 | 检查是否为 0x0000...fffFfF | 改为 Ethereum Sepolia 专用地址后重新部署回调合约 |
| UNISWAP_V2_PAIR_ADDR 填错 | echo $UNISWAP_V2_PAIR_ADDR | 必须是 PairCreated 事件中的地址 |
| 阈值高于当前汇率 | getReserves() 计算当前汇率 | 重新部署反应式合约，阈值低于当前汇率 |
| 反应式合约用了旧参数 | 查 Reactscan 上的合约事件 | 重新部署反应式合约并再次执行 swap |

**四、重要固定地址速查**

| 名称 | 地址 |
| Uniswap V2 Factory（Ethereum Sepolia） | 0x7E0987E5b3a30e3f2828572Bb659A548460a3003 |
| Uniswap V2 Router（Ethereum Sepolia） | 0xC532a74256D3Db42D0Bf7a0400fEFDbad7694008 |
| Callback Proxy（Ethereum Sepolia） | 0xc9f36411C9897e7F959D99ffca2a0Ba7ee0D7bDA |
| Callback Proxy（Base Sepolia） | 0xa6eA49Ed671B8a4dfCDd34E36b7a75Ac79B8A5a6 |
| Callback Proxy（Reactive Lasna） | 0x0000000000000000000000000000000000fffFfF |
...
```
echo "DEST_RPC: $DESTINATION_RPC"
```

```
echo "PROXY: $DESTINATION_CALLBACK_PROXY_ADDR"
```

```
echo "PAIR: $UNISWAP_V2_PAIR_ADDR"
```
...
```
cast call $TOKEN0_ADDR 'balanceOf(address)(uint256)' $UNISWAP_V2_PAIR_ADDR --rpc-url $DESTINATION_RPC
```

```
cast call $TOKEN1_ADDR 'balanceOf(address)(uint256)' $UNISWAP_V2_PAIR_ADDR --rpc-url $DESTINATION_RPC
```

**查看结果**

**\# Callback 合约事件（出现 Stop 表示成功）**

[**https://sepolia.etherscan.io/address/<CALLBACK\_ADDR>#events**](https://sepolia.etherscan.io/address/<CALLBACK_ADDR>#events)

**\# 反应式合约状态**

[**https://lasna.reactscan.net/address/<REACTIVE\_CONTRACT\_ADDR>**](https://lasna.reactscan.net/address/<REACTIVE_CONTRACT_ADDR>)
