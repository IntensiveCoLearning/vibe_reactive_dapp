...
## 先打卡，以防忘记了，下一步；准备做dapp，第一次做reactive contract dapp，接下来希望一切顺利吧！

solidity

```solidity
service.subscribe(
    SEPOLIA_CHAIN_ID,              // 监听哪条链？ → Sepolia
    stopOrderCallback,              // 监听哪个合约的事件？ → 你的 Callback 合约
    STOP_ORDER_CREATED_TOPIC_0,     // 监听什么事件？ → StopOrderCreated
    REACTIVE_IGNORE,                // 过滤 topic_1？ → 不过滤（任何 pair 都监听）
    REACTIVE_IGNORE,                // 过滤 topic_2？ → 不过滤（任何 orderId 都监听）
    REACTIVE_IGNORE                 // 过滤 topic_3？ → 不过滤
);
```

翻译成人话就是：“Reactive Network，请帮我盯着 Sepolia 链上我的 Callback 合约，只要它 emit 了 `StopOrderCreated` 事件，不管是哪个 pair、哪个 orderId，都通知我。”

`if (!vm)` 的作用是什么？`vm` 是 `AbstractReactive` 里的变量，在 Foundry 测试环境里为 `true`，在真实链上为 `false`。这个判断确保 `service.subscribe()` 只在真实部署时执行——这也是为什么 `forge script` 在本地模拟时会失败，因为 `!vm` 为 true，它试图调用 `service.subscribe()`，但本地没有系统合约 `0x...fffFfF`。
...
| 合约名称 | 部署链 | 核心职责 | 类比 |
| RescuableBase | 被 Callback 继承 | 紧急资金救援（ETH + ERC20） | 建筑消防通道 |
| Callback 合约 | Sepolia | 订单管理 + swap 执行 + 跨链回调接收 | 银行柜台 |
| Reactive 合约 | Lasna | 事件监听 + 价格判断 + 跨链触发 | 保安监控室 |

## 1.2 完整数据流（一个止损订单的生命周期）

| 阶段 | 操作 | 链 | 关键函数/事件 |
| ① 创建订单 | 用户调用 Callback | Sepolia | createStopOrder() → emit StopOrderCreated |
| ② Reactive 感知 | 监听到 Created 事件 | Lasna | react() → _processOrderCreated() |
| ③ 动态订阅 | 自我回调订阅 Pair 的 Sync | Lasna→Lasna | _requestPairSubscription() → subscribeToPair() |
| ④ 价格监控 | 每次 swap 触发 Sync | Sepolia→Lasna | react() → _processSyncEvent() |
| ⑤ 条件判断 | 计算价格是否满足阈值 | Lasna | _isPriceConditionMet() |
| ⑥ 跨链触发 | emit Callback 到 Sepolia | Lasna→Sepolia | _triggerExecution() → emit Callback |
| ⑦ 二次验证 | Callback 读实时价格再确认 | Sepolia | executeStopOrder() → _isPriceConditionMet() |
| ⑧ 执行 swap | 代币从用户钱包经过 Callback 到 Router | Sepolia | _executeSwap() → swapExactTokensForTokens() |
| ⑨ 状态清理 | Reactive 监听到 Executed 事件 | Sepolia→Lasna | processOrderExecuted() → decrementPairCount() |

## 1.3 五大核心设计模式

| 模式一：双重价格验证（Belt and Suspenders）Reactive 在 Lasna 上根据 Sync 事件判断价格（第一道门），Callback 在 Sepolia 上读实时 reserve 再验证（第二道门）。原因：事件从 Sepolia 传到 Lasna 有延迟，延迟期间价格可能已经变化。 |

| 模式二：自我回调订阅（Self-Callback）react() 函数的上下文是 vmOnly，不能直接调 service.subscribe()（需要 rnOnly）。解决方案：emit Callback 给自己（address(this)），Reactive Network 收到后回调 subscribeToPair()，此时满足 rnOnly。 |

| 模式三：事件驱动的状态同步跨链无法直接读取另一条链的 storage。Callback 每次状态变化 emit 事件（Created/Cancelled/Executed/Paused/Resumed），Reactive 监听并维护镜像副本。 |

| 模式四：引用计数资源管理pairOrderCount 跟踪每个 Pair 的活跃订单数。每创建一个 +1，每取消/执行一个 -1。归零时自动取消订阅 Sync 事件，避免浪费资源监听没有订单的 Pair。 |

| 模式五：优雅降级executeStopOrder() 中，如果 owner 余额或授权不够全额执行，自动降低到实际可用数量。不是「要么全做、要么不做」，而是「能做多少做多少」。 |

  

# 二、合约一：RescuableBase.sol

定位：紧急资金救援的抽象基类，不能独立部署，必须被继承。

## 2.1 关键知识点：abstract 和 virtual
...
**abstract** 表示这个合约不能独立部署。**virtual** 表示这个函数可以（或必须）被子类覆盖。没有函数体的 virtual 函数 = 强制子类实现。

这是设计模式中的**「模板方法模式」**——父类定义流程（怎么转账），子类定义细节（转给谁、谁有权限）。

## 2.2 关键知识点：ETH 和 ERC20 是完全不同的系统

| 你的疑问：为什么要写两个 rescue 函数？ETH 是以太坊的原生货币，内嵌在 EVM 底层，用 address(this).balance 查余额，用 .call{value:...} 转账。ERC20 是别人部署的智能合约，用 IERC20(token).balanceOf() 查余额，用 safeTransfer() 转账。虽然都是「把钱转出去」，但操作方式完全不同。类比：ETH = 口袋里的现金（直接递出去），ERC20 = 银行存款（必须去柜台办理）。 |

## 2.3 关键知识点：emit 不是转账

| 你的疑问：emit ETHRescued(recipient, rescueAmount) 是把钱转给我吗？不是！emit 只是「记一笔日志」。真正转账的是上面那行 .call{value: rescueAmount}。emit 相当于银行转账后打印的回执单，不改变任何人余额，只在区块链日志里写一条记录。Reactive 合约正是靠监听这些 emit 出来的事件来感知其他链上发生了什么。 |

## 2.4 关键代码：ETH 转账的最佳实践

// ✅ 推荐写法：没有 gas 限制，兼容合约接收者

(bool success,) = payable(recipient).call{value: rescueAmount}("");

require(success, "ETH transfer failed");

// ❌ 不推荐：transfer() 限制 2300 gas，多签钱包可能不够用

// payable(recipient).transfer(rescueAmount);

(bool success,) 里的逗号：.call() 返回 (bool, bytes)，我们只要 bool，用逗号丢弃 bytes。

## 2.5 关键代码：SafeERC20 的必要性

using SafeERC20 for IERC20;

// safeTransfer 比 transfer 更安全

// 因为某些代币（如 USDT）的 transfer() 不返回 bool

// 直接调用会 revert，SafeERC20 自动处理了兼容性

IERC20(token).safeTransfer(recipient, rescueAmount);

## 2.6 Callback 合约如何覆盖 RescuableBase
...
# 三、合约二：UniswapDemoStopTakeProfitCallback.sol

定位：部署在 Sepolia 上的「执行者」，负责订单管理和实际的 swap 交易。

## 3.1 多重继承

contract UniswapDemoStopTakeProfitCallback is AbstractCallback, RescuableBase {

    // AbstractCallback → 提供 authorizedSenderOnly 修饰符

    //                     保证只有 Callback Proxy 能调用 executeStopOrder

    // RescuableBase    → 提供紧急资金救援能力

}

## 3.2 事件设计：indexed 与 data 的分工

event StopOrderCreated(

    address indexed pair,      // → topic\_1（可被高效过滤）

    uint256 indexed orderId,   // → topic\_2（可被高效过滤）

    bool sellToken0,           // ┐

    address tokenSell,         // │

    address tokenBuy,          // │→ 打包在 [log.data](http://log.data) 里

    uint256 amount,            // │   需要 abi.decode 解码
...
indexed 参数进 topic（最多 3 个），非 indexed 参数进 data。Reactive 合约通过 topic\_0 识别事件类型，通过 topic\_1/topic\_2 获取关键 ID，通过 abi.decode([log.data](http://log.data)) 获取详细参数。

## 3.3 自定义错误 vs require 字符串

// 新方式（省 gas）：只存储 4 字节选择器 + 参数
...
## 3.4 订单状态机（5 种状态的转换规则）

| 起始状态 | 操作 | 目标状态 | 谁能触发 |
| Active | pauseStopOrder() | Paused | owner |
| Paused | resumeStopOrder() | Active | owner |
| Active / Paused | cancelStopOrder() | Cancelled | owner |
| Active | executeStopOrder() | Executed | Callback Proxy（跨链） |
| Active | 重试超限 / 余额不足 | Failed | 系统自动 |

Executed、Cancelled、Failed 是终态，到达后无法再转换。

## 3.5 immutable vs constant vs 普通变量

address public immutable owner;   // 构造函数里赋值一次，之后不可改

                                  // 编译器嵌入字节码，读取不需要访问 storage

uint256 private constant DEADLINE\_OFFSET = 300;  // 编译期确定，不可改

                                                 // 比 immutable 更省 gas

StopOrder\[\] public stopOrders;  // 普通状态变量，可以随时修改

                                // 读写都需要访问 storage（最贵的操作）

## 3.6 createStopOrder：四层安全检查

// 第一层：参数验证

require(pair != address(0), "Invalid pair address");

require(amount >= MIN\_AMOUNT, "Amount too small");

// 第二层：从链上权威来源获取代币地址（不信任用户输入）
...
## 3.7 executeStopOrder：六层防御

| 检查层 | 检查内容 | 失败结果 |
| ① 状态检查 | order.status == Active? | revert OrderNotActive |
| ② 二次价格验证 | Sepolia 实时 reserve 是否仍满足条件 | revert PriceConditionNotMet |
| ③ 冷却检查 | 距上次尝试是否超过 30 秒 | 静默返回（不 revert） |
| ④ 重试上限 | retryCount < 3? | 标记 Failed + revert |
| ⑤ 余额/授权 | owner 余额和授权够吗（可降级） | 标记 Failed + revert |
| ⑥ 执行 swap | _executeSwap() | revert（交易回滚） |

## 3.8 \_executeSwap：代币流转的四步

// 步骤 1：从 owner 钱包把代币转到 Callback 合约

tokenSell.safeTransferFrom(owner, address(this), amount);

// 步骤 2：Callback 合约授权给 Router

tokenSell.forceApprove(address(router), amount);

// forceApprove 比 approve 更安全（兼容 USDT 等先归零再设值的要求）

// 步骤 3：Router 执行 swap

uint256\[\] memory amounts = router.swapExactTokensForTokens(

    amount, 0, path, address(this), block.timestamp + DEADLINE\_OFFSET

);

// amountOutMin = 0 → 接受任意滑点（止损场景：赶紧卖比卖好价更重要）

// 步骤 4：把换回的代币发给 owner

tokenBuy.safeTransfer(owner, amountOut);

Callback 合约是「中转站」——代币经过它但不会留在里面。如果意外留下了，RescuableBase 可以取回。

## 3.9 价格计算核心算法

// currentPrice = (reserve\_买入代币 × coefficient) / reserve\_卖出代币

currentPrice = Math.mulDiv(uint256(reserve1), coefficient, uint256(reserve0));

// Math.mulDiv(a, b, c) = (a × b) / c

// 内部用 512 位防溢出，比直接 a \* b / c 更安全

| 你的疑问：threshold=1500 意味着涨了 50% 吗？是的。threshold / coefficient = 目标价格。coefficient=1000, threshold=1500 → 目标价格 1.5，相比初始 1.0 涨了 50%。coefficient=1000, threshold=900 → 目标价格 0.9，跌了 10%。coefficient 是「精度放大器」，因为 Solidity 没有小数。 |

  

# 四、合约三：UniswapDemoStopTakeProfitReactive.sol

定位：部署在 Lasna 上的「大脑」，负责盯着 Sepolia 的事件并在条件满足时发出跨链触发信号。

## 4.1 事件签名常量：topic\_0 的「指纹」

uint256 private constant UNISWAP\_V2\_SYNC\_TOPIC\_0 =

    0x1c411e9a96e071241c2f21f7726b17ae89e3cab4c78be50e062b03a9fffbbad1;

// 来源：keccak256("Sync(uint112,uint112)")

// 每个事件的 topic\_0 = 事件签名的 keccak256 哈希

// Reactive 合约通过匹配 topic\_0 来识别「发生了什么事件」

// 类比：信号弹的颜色——红色=撤退，绿色=前进，不用读字就知道意思

## 4.2 TrackedOrder vs StopOrder：精简的镜像

Reactive 合约的 TrackedOrder 只保留判断价格所需的字段，省略了 tokenSell、tokenBuy、amount 等执行细节。原则：每个合约只存储自己需要的数据。多存既浪费 gas，也增加同步复杂度。

## 4.3 四个 mapping 的协作查询系统

| mapping | 类型 | 回答的问题 |
| trackedOrders | uint256 → TrackedOrder | 这个订单的详细信息是什么？ |
| pairOrders | address → uint256[] | 这个池子上挂了哪些订单？ |
| pairOrderCount | address → uint256 | 这个池子还有多少活跃订单？ |
| subscribedPairs | address → bool | 这个池子的 Sync 事件正在被监听吗？ |

## 4.4 构造函数：一口气订阅 5 个事件

if (!vm) {

    // vm = true 时是 Foundry 测试环境，跳过订阅

    // vm = false 时是真实链上，执行订阅

    service.subscribe(

        SEPOLIA\_CHAIN\_ID,              // 监听哪条链

        stopOrderCallback,              // 监听哪个合约

        STOP\_ORDER\_CREATED\_TOPIC\_0,     // 监听什么事件

        REACTIVE\_IGNORE,                // 不过滤 topic\_1/2/3
...
    // ... 同样订阅 Cancelled、Executed、Paused、Resumed

}

| 关键细节构造函数里只订阅了 Callback 的 5 个生命周期事件，没有订阅 Uniswap Pair 的 Sync 事件。Sync 的订阅是动态的——有了具体的订单才知道要监听哪个 Pair。这也是 if(!vm) 导致 forge script 失败的根因——本地没有系统合约 0x...fffFfF，subscribe() 在模拟阶段就 revert。 |

## 4.5 react()：唯一的外部入口（事件路由器）

function react(LogRecord calldata log) external vmOnly {

    if (log.\_contract == stopOrderCallback) {

        // 来自 Callback 合约的事件 → 处理订单生命周期

        \_processStopOrderEvent(log);

    } else if (log.topic\_0 == UNISWAP\_V2\_SYNC\_TOPIC\_0

              && subscribedPairs\[log.\_contract\]) {

        // 来自已订阅 Pair 的 Sync 事件 → 处理价格监控
...
// indexed 参数在 topic 里，需要类型转换

address pair = address(uint160(log.topic\_1));

// topic 是 uint256（32字节），地址是 20 字节，需要先转 uint160 再转 address

// 非 indexed 参数在 data 里，需要 abi.decode

(bool sellToken0,,,, uint256 coefficient, uint256 threshold, OrderType orderType) =

    abi.decode([log.data](http://log.data), (bool, address, address, uint256, uint256, uint256, OrderType));

// 三个逗号 = 跳过 tokenSell、tokenBuy、amount（不需要它们）

| 注意：不同事件的 orderId 在不同的 topic 位置StopOrderCancelled(uint256 indexed orderId) → orderId 在 topic_1。StopOrderExecuted(address indexed pair, uint256 indexed orderId, ...) → orderId 在 topic_2。indexed 参数按声明顺序依次填入 topic_1、topic_2、topic_3，必须对照原始事件定义才能正确提取。 |

## 4.7 \_processSyncEvent：价格监控核心引擎

function \_processSyncEvent(LogRecord calldata log) internal {

    Reserves memory reserves = abi.decode([log.data](http://log.data), (Reserves));

    uint256\[\] storage orderIds = pairOrders\[pair\];

    for (uint256 i = 0; i < orderIds.length; i++) {

        TrackedOrder storage order = trackedOrders\[orderIds\[i\]\];

        if (order.status != OrderStatus.Active) continue;  // 跳过非活跃

        if (/\* 冷却中 \*/) continue;                         // 5 分钟内不重复

        if (order.triggerCount >= 5) { Failed; continue; }  // 超过 5 次
...
一次 Sync 事件可以同时触发同一个 Pair 上的多个订单。

## 4.8 \_triggerExecution：发射「跨链导弹」

bytes memory payload = abi.encodeWithSignature(

    "executeStopOrder(address,uint256)",

    address(0),  // sender 参数，Callback 合约会忽略
...
    SEPOLIA\_CHAIN\_ID,     // 发到 Sepolia

    stopOrderCallback,     // 调用 Callback 合约

    CALLBACK\_GAS\_LIMIT,    // 给 1000000 gas

    payload                // 调用 executeStopOrder(0x0, orderId)

);

Reactive Network 的中继节点捕获这个 Callback 事件后，通过 Sepolia 的 Callback Proxy（0xc9f364...）执行 payload。

## 4.9 自我回调订阅机制（最巧妙的设计）

// 步骤 1：在 react() 上下文里，不能直接调 service.subscribe()

// 所以 emit Callback 给自己

emit Callback(

    REACTIVE\_CHAIN\_ID,   // 目标链：Lasna（不是 Sepolia！）

    address(this),        // 目标合约：自己

    CALLBACK\_GAS\_LIMIT,

    abi.encodeWithSignature("subscribeToPair(address,address,uint256)", ...)

);

// 步骤 2：Reactive Network 收到后回调 subscribeToPair()

function subscribeToPair(...) external rnOnly {

    // 此时满足 rnOnly 的要求

    service.subscribe(chainId, pair, UNISWAP\_V2\_SYNC\_TOPIC\_0, ...);

}

| 为什么不能直接调用？service.subscribe() 需要 rnOnly 修饰符（Reactive Network 直接调用）。但 react() 函数的修饰符是 vmOnly（Reactive VM 调用），两者是不同的执行上下文。通过自我回调绕了一圈，让 Reactive Network「直接调用」subscribeToPair()，满足了 rnOnly 的要求。 |
...
            _requestPairUnsubscription(pair, SEPOLIA_CHAIN\_ID);
...
类比操作系统的文件句柄：多个进程打开同一文件，引用计数 +1。最后一个关闭时，计数归零，系统释放资源。
...
## 一、解决的核心问题：Reactive 合约部署失败

### 问题描述

之前用 `forge script` 部署 Reactive 合约到 Lasna 一直失败，尝试了多种方案都不行。

### 根本原因

`forge script` 的工作流是：**先本地模拟 → 模拟成功后才广播到链上**。

Reactive 合约的构造函数里调用了 `service.subscribe()`，这个函数会访问 Lasna 的系统预编译合约 `0x0000...fffFfF`。这个合约只存在于 Lasna 链上，本地 EVM 里没有，所以模拟阶段必定 revert，交易永远到不了链上。

### 尝试过的方案及结果

| 方案 | 结果 | 原因 |
| --- | --- | --- |
| forge script --rpc-url | ❌ | 本地模拟找不到预编译合约 |
| forge script --skip-simulation | ❌ | 仍然执行了模拟 |
| forge script + vm.deal | ❌ | 不解决预编译合约缺失问题 |
| forge script --fork-url | ❌ | 能复制状态但不能复制预编译执行逻辑 |
| forge create --broadcast --legacy | ✅ | 直接发送交易到链上，跳过本地模拟 |

### 最终结论

| 部署方式 | Sepolia 合约 | Lasna Reactive 合约 |
| --- | --- | --- |
| forge script | ✅ 可以 | ❌ 永远不行 |
| forge create --broadcast | ✅ 可以 | ✅ 唯一可行方案 |
...
| 合约 | 链 | 地址 | 部署方式 |
| --- | --- | --- | --- |
| Callback  回调 | Sepolia | 0xEEcf30ACFff83f2225b31f878a9291b2787B4fD9 | forge script |
| Reactive  反应式 | Lasna | 0x01e914F1ca6F3816A2D8364dF230706F19636099 | forge create --broadcast |
| TokenA (手动) | Sepolia | 0xb1fA071DC0BCA190D236c0f6f3589C4b6b4a3973 | forge create |
| TokenB (手动) | Sepolia | 0xED1D31a38841AE360395f2EDE616A8d3dE649C37 | forge create |
| Pair (手动) | Sepolia | 0x37499ab467f05d871c05358516746f00cf428497 | 自动创建 |
| TokenA (脚本) | Sepolia | 0x902082dAaaA2bA3198761245112D1B2b180761a1 | forge script |
| TokenB (脚本) | Sepolia | 0xF08f5fEcFB7AF605B73c168f4132Bb0F1c099BD3 | forge script |

### 手动测试流程验证（全部成功）

```
第 1 步：部署 TokenA + TokenB → ✅
第 2 步：approve 给 Router + addLiquidity 创建池子 → ✅
第 3 步：approve TokenA 给 Callback → ✅
第 4 步：createStopOrder（价格 ≤ 0.9 卖出 10 个 TKA）→ ✅
第 5 步：砸盘 swap 30 TKA 进池子 → ✅
第 6 步：等待 → Reactive 自动检测 → Callback 自动执行 → ✅
第 7 步：验证 status=3（Executed），TokenB 余额增加 → ✅
```
...
```
初始：100 TKA, 100 TKB
→ 添加流动性 -50, -50 → 50 TKA, 50 TKB
→ 砸盘 swap -30 TKA, +18.75 TKB → 20 TKA, 68.75 TKB
→ 止损自动执行 -10 TKA, +3.47 TKB → 10 TKA, 72.22 TKB ✅
```
...
### 确认了 Sepolia + Lasna 完全支持本项目

| 路径 | 用途 | 状态 |
| --- | --- | --- |
| Sepolia → Lasna | 事件监听（Sync, StopOrderCreated）事件监听（同步，订单创建停止） | ✅ 支持 |
| Lasna → Sepolia | 触发回调（executeStopOrder） | ✅ 有 Callback Proxy |
| Lasna → Lasna | 自我回调（动态订阅 Pair） | ✅ 支持 |

### 确认了 Uniswap V2 Router 在 Sepolia 可用

-   地址：0xC532a74256D3Db42D0Bf7a0400fEFDbad7694008
    
-   Etherscan 已验证，176,000+ 笔交易，持续活跃
...
| 参数 | 值 |
| --- | --- |
| Sepolia Chain ID  Sepolia 链 ID | 11155111 |
| Lasna Chain ID  Lasna 链 ID | 5318007 |
| Sepolia Callback Proxy  Sepolia 回调代理 | 0xc9f36411C9897e7F959D99ffca2a0Ba7ee0D7bDA |
| Lasna System Contract  Lasna 系统合约 | 0x0000000000000000000000000000000000fffFfF |
| Lasna RPC | https://lasna-rpc.rnk.dev/ |
| Uniswap V2 Router  Uniswap V2 路由器 | 0xC532a74256D3Db42D0Bf7a0400fEFDbad7694008 |
| Uniswap V2 Factory  Uniswap V2 工厂 | 0x7E0987E5b3a30e3f2828572Bb659A548460a3003 |

* * *

## 四、学会了 Foundry forge script 脚本编写
...
contract MyScript is Script {
    function run() external {
        vm.startBroadcast();    // 开始发送交易到链上
        // 你的操作写在这里
        vm.stopBroadcast();     // 停止发送
    }
}
```
...
```powershell
forge script script/脚本名.s.sol `
  --rpc-url $env:DESTINATION_RPC `
  --private-key $env:ORIGIN_PRIVATE_KEY `
  --broadcast `
  --legacy
```
...
import "forge-std/Script.sol";
import "../src/demos/uniswap-v2-stop-order/UniswapDemoToken.sol";

contract Myscript is Script {
    function run() external {
        vm.startBroadcast();
        UniswapDemoToken tokenA = new UniswapDemoToken("TokenA", "TKA");
...
2\. SetupPool.s.sol — 创建池子 + 授权 + 创建止损订单

学会了在脚本里调用已部署合约的函数：

-   `IERC20(tokenA).approve(router, 50 ether)` — 调用 ERC20 接口
    
-   `IUniswapV2Router02(router).addLiquidity(...)` — 调用 Router 接口
    
-   `IUniswapV2Factory(factory).getPair(...)` — 查询 Pair 地址
    
-   `UniswapDemoStopTakeProfitCallback(payable(addr)).createStopOrder(...)` — 调用合约函数
    

3\. TriggerStopLoss.s.sol — 砸盘触发止损  
3\. TriggerStopLoss.s.sol — 触发止损
...
contract TriggerStopLoss is Script {
    address tokenA = 0x902082dAaaA2bA3198761245112D1B2b180761a1;
    address tokenB = 0xF08f5fEcFB7AF605B73c168f4132Bb0F1c099BD3;
    ...
    function run() external {
        ...
        vm.startBroadcast();
        IERC20(tokenA).approve(router, 30 ether);
...
| 命令 | 用途 | 需要模拟？ |
| --- | --- | --- |
| forge build | 只编译代码，不部署 | 否 |
| forge create --broadcast | 直接部署合约到链上 | 否（直接发送） |
| forge script --broadcast | 运行 Solidity 脚本部署/交互 | 是（先模拟后广播） |
| cast send | 发送单笔交易（写操作） | 否 |
| cast call | 读取链上数据（不消耗 gas） | 否 |

### Solidity 中 ether 关键字  
Solidity 中的 ether 关键字

solidity

```solidity
50 ether = 50000000000000000000 = 50 × 10^18
// 只是单位换算，不是 50 个 ETH
// 在 ERC20 场景里表示 50 个代币（18 位小数）
```
...
每次部署 nonce 不同，所以地址不同。和 Hardhat 本地网络重启后地址相同不一样。

### forge script 注意事项  锻造脚本注意事项

-   `--silent` 不代表没执行，只是不显示输出，要小心
    
-   `forge script` 模拟失败时，所有交易都不会广播（原子性）
    
-   `vm.startBroadcast()` 和 `vm.stopBroadcast()` 之间的代码才会发送到链上
    
-   `console.log()` 只在本地显示，不上链
...
| 步骤 | 测试网 | 主网 |
| --- | --- | --- |
| 部署代币 | 需要（自己造假代币） | 不需要（代币已存在） |
| 创建池子 + 流动性 | 需要（搭建测试环境） | 不需要（池子已存在） |
| 部署 Callback + Reactive | 需要 | 需要 |
| approve 给 Callback  批准给 Callback | 需要 | 需要 |
| createStopOrder  创建止损单 | 需要 | 需要 |
| 触发价格变化 | 手动砸盘 | 真实市场自动发生 |

### 跨链的理解

Reactive Network 只做"监听和触发"，不碰你的代币。实际的 swap 完全在同一条链上完成。两个代币必须在同一条链上，不涉及跨链兑换。

* * *

## 六、待改进的事项

1.  **脚本地址硬编码问题** — 应改为从 `.env` 文件读取，用 `vm.envAddress("TOKEN_A")`
    
2.  **脚本拆分建议** — DeployAll / SetupPool / TriggerStopLoss 三个脚本分工明确
    
3.  **Foundry Solidity 脚本限制** — Lasna 上的 Reactive 合约只能用 `forge create`，无法用 `forge script`
...
## 被这个自动止盈止损搞得头有点大，不确定今天是否可以搞完，先打卡

## 还是没有解决，明天问问大佬；今天先结束了
...
-   紧急救援合约，当代币卡在合约里时可以取回
    
-   设计思路：基类只管"怎么取"，子类决定"谁能取"
    
-   `_rescueRecipient()` 抽象函数由子类实现，返回 owner 地址
    
-   `safeTransfer` 防止不标准代币静默失败
    
-   rescue 函数是应急备用通道，不是卖出流程的一部分
    

**UniswapDemoStopTakeProfitCallback.sol（Sepolia）**

-   订单生命周期管理：创建/暂停/恢复/取消
    
-   `executeStopOrder()` 被 Reactive Network 自动调用
    
-   `_executeSwap()` 完整卖出流程：transferFrom → approve Router → swap → transfer回给你  
    `_executeSwap()` 完整卖出流程：transferFrom → approve Router → swap → transfer 回给你
    
-   `_isPriceConditionMet()` 用 `Math.mulDiv` 防止溢出
    
-   状态机：Active / Paused / Cancelled / Executed / Failed  
    状态机：活动 / 暂停 / 取消 / 执行 / 失败
    

**UniswapDemoStopTakeProfitReactive.sol（Lasna）**

-   构造函数订阅 Callback 合约的5种生命周期事件
    
-   `react()` 双分支：来自 Callback 合约 → 处理订单状态；来自 Uniswap Pair → 判断价格
    
-   动态订阅/取消订阅：第一个订单创建时订阅 Pair，所有订单完成后取消订阅
    
-   自我回调：`emit Callback(REACTIVE_CHAIN_ID, address(this), ...)` 回调自己来修改订阅
    
-   `ReactVM` 限制：不能跨链读取数据，必须通过监听事件同步状态
...
-   Uniswap Router 是 Uniswap 官方合约，不是 Reactive 的
    
-   `{value: 0.5 ether}` 里的 ether 只是单位，在 Lasna 上就是 lREACT
    
-   approve 授权链：你 → Callback合约 → Router → Uniswap池子
    
-   token0/token1 由 Uniswap 按地址大小排序，不是你决定的
    
-   topic\_0 = 事件签名的 keccak256，和合约地址无关，只和事件定义有关
...
-   `forge script` 部署合约（配合 `--broadcast`）
    
-   `cast balance` 查询余额
    
-   `cast block-number` 测试 RPC 连接
    
-   `cast wallet address` 验证私钥格式
...
-   Foundry 1.5.x 的 `forge create` 默认 dry run，需要 `--broadcast`
    
-   但 `forge create --broadcast` 在此项目里有问题，改用 `forge script` 成功
    
-   Windows PowerShell 用 `$env:变量名="值"` 设置环境变量
...
✅ Foundry 安装正常
✅ 项目 clone 和编译成功
✅ Sepolia RPC 和 Lasna RPC 连接正常
✅ Callback 合约成功部署到 Sepolia
   地址：0xEEcf30ACFff83f2225b31f878a9291b2787B4fD9
✅ 修正了两个 chain_id：
   SEPOLIA_CHAIN_ID = 11155111
   REACTIVE_CHAIN_ID = 5318007
✅ 发现 Callback 合约需要附带 value: 0.02 ether
```
...
```
❌ Reactive 合约部署失败
   原因：forge script 本地模拟时
         Lasna 预编译合约 0x0000...0064 不存在
         subscribe() 调用在模拟阶段就 revert
