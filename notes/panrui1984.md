...
Reactive Library 提供了一套接口让智能合约变成一个\*\*“跨链事件监听器 + 自动执行机器人”\*\*。

开发一个响应式智能合约的典型步骤：

1.  **继承库文件：** 导入并继承 AbstractReactive。
    
2.  **配置订阅 (Subscribe)：** 在合约的构造函数 (constructor) 中，使用 subscribe(chainId, address, topic0) 方法注册你需要监听的事件。
    
3.  **实现自动化逻辑 (React)：** 重写 (override) react() 函数，接收事件传来的数据（如 topic\_1, topic\_2, data 等），并编写你要自动执行的业务代码
